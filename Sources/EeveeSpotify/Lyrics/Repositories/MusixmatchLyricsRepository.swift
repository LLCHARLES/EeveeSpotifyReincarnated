    ...
    func getLyrics(_ query: LyricsSearchQuery, options: LyricsOptions) throws -> LyricsDto {
        ...
        if let trackSubtitlesGet = macroCalls["track.subtitles.get"] as? [String: Any],
            ...
            let subtitles = try? JSONDecoder().decode(
                [MusixmatchSubtitle].self, from: Data(subtitleBody.utf8)  // 改进：使用 Data(subtitleBody.utf8)
            ),
            !subtitles.isEmpty  // 改进：添加空数组检查
        {
            ...
            subtitleLines.append(
                LyricsLineDto(
                    words: "",
                    startTimeMs: Int64((subtitles.last?.time.total ?? 0) * 1000),  // 改进：安全处理 last
                    syllables: nil
                )
            )
            ...
            if selectedLanguage != subLanguage,
                ...
                let subtitlesTranslated = try? JSONDecoder().decode(
                    [MusixmatchSubtitle].self, from: Data(subtitleTranslatedBody.utf8)  // 改进：使用 Data(subtitleTranslatedBody.utf8)
                )
            {
